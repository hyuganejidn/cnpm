...
const formValidator = new FormValidator([
  ...
  },
  {
    ...
    method: validator.isEmpty,
    validWhen: false,
    message: "Vui lòng nhập mô tả địa điểm"
  },
  {
    field: "price",
    method: validator.isEmpty,
  ...
  },
  {
    ...
    method: validator.isEmpty,
    validWhen: false,
    message: "Vui lòng nhập các dịch vụ"
  },
]);
...
const HotelsForm = (props) => {
  ...
  const [place, setPlace] = useState({
    name: '', content: '', street_address: '', extended_address: '', mobile: '', property_amenities: '', price: '', category_id: 1
  })
    ...
    <div>
      <Row>
        <Col >
          ...
          <Form.Group as={Col}>
            <Form.Label className="title-weight-400">Các dịch vụ* :</Form.Label>
            <Form.Control placeholder="Các dịch vụ" type='number' value={place.property_amenities} className='input-number' onChange={(e) => onChange(e, 'property_amenities')} />
            <div className="error">
              {validationResult.property_amenities && validationResult.property_amenities.isInvalid && validationResult.property_amenities.message}
            </div>
          </Form.Group>
          <Form.Group as={Col} lg="6">
            <Form.Label className="title-weight-400">Giá tiền* :</Form.Label>
            <Form.Control placeholder="Giá tiền" type='number' value={place.price} className='input-number' onChange={(e) => onChange(e, 'price')} />
            <div className="error">
              {validationResult.price && validationResult.price.isInvalid && validationResult.price.message}
            </div>
          </Form.Group>
        </Col>
